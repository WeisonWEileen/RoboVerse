...
class DinoV3Encoder(nn.Module):
    def __init__(self, device="cuda"):
        super().__init__()
        # 1) 从 torch.hub 加载 DINOv3 ViT-S/16
        #   第一次会自动从 github 下权重
        self.backbone = torch.hub.load(
            "facebookresearch/dinov3",   # repo
            "dinov3_vits16",             # model name
            source="github",             # 显式指定
            pretrained=True,
        ).to(device)
        self.backbone.eval()            # 通常作为 frozen encoder 用
        for p in self.backbone.parameters():
            p.requires_grad_(False)

        self.device = device

        # DINO 系列一般有 forward_features，可以直接拿 CLS token
        self.use_forward_features = hasattr(self.backbone, "forward_features")

        # 这里可以用你自己的 norm；先给一个 ImageNet 风格的
        self.preprocess = transforms.Compose([
            # 输入如果是 0–255 uint8，会在 forward 里转 float /255
            transforms.Resize((224, 224)),   # 可改成 (96,128) 但 224 比较稳
        ])

        # 只是为了算输出维度
        with torch.no_grad():
            dummy = torch.zeros(1, 3, 224, 224).to(device)
            feats = self._encode_raw(dummy)
        self.out_dim = feats.shape[1]

    @torch.no_grad()
    def _encode_raw(self, x: torch.Tensor) -> torch.Tensor:
        """x 已经在 device 上，并且是 float32, [0,1]"""
        if self.use_forward_features:
            out = self.backbone.forward_features(x)
            # DINOv2 / v3 通常返回 dict，x_norm_clstoken 是 CLS 向量
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        x: (B, 3, H, W)，可以是 0–1 或 0–255
        返回: (B, D)
        """
        x = x.to(self.device)
        if x.dtype != torch.float32:
            x = x.float()
        if x.max() > 1.5:      # 粗暴判断是 0–255
            x = x / 255.0

        # resize 到 backbone 习惯的分辨率
        x = self.preprocess(x)
        feats = self._encode_raw(x)
        return feats
...
class ActorCriticDIVO3RNN(ActorCritic):
    is_recurrent = True
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # load divo3 vision encoder
        self.vision_encoder =  DinoV3Encoder()
