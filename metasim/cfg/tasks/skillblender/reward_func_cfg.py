...
def reward_base_height(states: EnvState, robot_name: str, cfg: BaseRLTaskCfg) -> torch.Tensor:
    ...
    measured_heights = torch.sum(
        states.robots[robot_name].extra["rigid_body_states"][:, cfg.feet_indices, 2] * stance_mask,
        dim=1,
...
def reward_knee_distance(states: EnvState, robot_name: str, cfg: BaseRLTaskCfg) -> torch.Tensor:
    ...
    """
    knee_pos = states.robots[robot_name].extra["rigid_body_states"][:, cfg.knee_indices, :2]
...
def reward_elbow_distance(states: EnvState, robot_name: str, cfg: BaseRLTaskCfg) -> torch.Tensor:
    ...
    """
    elbow_pos = states.robots[robot_name].extra["rigid_body_states"][:, cfg.elbow_indices, :2]
...
def reward_foot_slip(states: EnvState, robot_name: str, cfg: BaseRLTaskCfg) -> torch.Tensor:
    ...
    foot_speed_norm = torch.norm(
        states.robots[robot_name].extra["rigid_body_states"][:, cfg.feet_indices, 10:12], dim=2
    )
