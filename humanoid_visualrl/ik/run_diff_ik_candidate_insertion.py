...
class TableTopSceneCfg(InteractiveSceneCfg):
    ...
    cube = RigidObjectCfg(
        ...
        init_state=RigidObjectCfg.InitialStateCfg(
            pos=(0.488, 0.142, 0.5650),
            rot=(0.9677, 0.0, 0.0, -0.2522),  # quaternion (w, x, y, z)
        ),
    )

    insertion_female_box = RigidObjectCfg(
        prim_path="{ENV_REGEX_NS}/InsertionFemaleBox",
        spawn=sim_utils.UsdFileCfg(
        ...
        init_state=RigidObjectCfg.InitialStateCfg(
            pos=(0.55, 0.1, 0.51),
            rot=(0.8660254, 0.0, 0.0, 0.5),  # quaternion (w, x, y, z)
        ),
    )
...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    ...
    robot = scene["robot"]
    cube = scene["cube"]
    insertion_female_box = scene["insertion_female_box"]

    # Initialize keyboard input (only if GUI is available)
    keyboard_state = {
        "x_pos": False,  # W key
        "x_neg": False,  # S key
        "y_pos": False,  # A key
    ...
    try:
        ...
        def on_keyboard_event(event, *args, **kwargs):
            ...
            key_map = {
                carb.input.KeyboardInput.W: ("x_pos", True),
                carb.input.KeyboardInput.S: ("x_neg", True),
                carb.input.KeyboardInput.A: ("y_pos", True),
                carb.input.KeyboardInput.D: ("y_neg", True),
                carb.input.KeyboardInput.R: ("z_pos", True),
                carb.input.KeyboardInput.F: ("z_neg", True),
                carb.input.KeyboardInput.Q: ("yaw_pos", True),
                carb.input.KeyboardInput.E: ("yaw_neg", True),
            }

            if event.type == carb.input.KeyboardEventType.KEY_PRESS:
                if event.input in key_map:
                    key_name, _ = key_map[event.input]
                    keyboard_state[key_name] = True
            elif event.type == carb.input.KeyboardEventType.KEY_RELEASE:
                if event.input in key_map:
                    key_name, _ = key_map[event.input]
                    keyboard_state[key_name] = False
            return True

        keyboard_sub = input_interface.subscribe_to_keyboard_events(keyboard, on_keyboard_event)
        print("[INFO]: Keyboard controls enabled:")
        print("  W/S - Move cube X +/-")
        print("  A/D - Move cube Y +/-")
        print("  R/F - Move cube Z (height) +/-")
        print("  Q/E - Rotate cube Yaw +/-")
    except Exception as e:
        print(f"[WARNING]: Keyboard input not available (headless mode?): {e}")
    ...
    def yaw_to_quat(yaw):
        ...
        return torch.stack([cy, torch.tensor(0.0, device=yaw.device), torch.tensor(0.0, device=yaw.device), sy], dim=-1)

    # Helper function to extract yaw from quaternion
    def quat_to_yaw(quat):
        """Extract yaw angle from quaternion (w, x, y, z)."""
        # For quaternion (w, x, y, z), yaw around z-axis
        sin_yaw = 2.0 * (quat[0] * quat[3] + quat[1] * quat[2])
        cos_yaw = 1.0 - 2.0 * (quat[2] * quat[2] + quat[3] * quat[3])
        return torch.atan2(sin_yaw, cos_yaw)

    # Initialize cube control state (relative to env_origins)
    # Initial pose: [0.488, 0.142, 0.5650, 0.9677, 0.0, 0.0, -0.2522] (pos + quat)
    cube_control_pos = torch.tensor([0.488, 0.142, 0.5650], device=sim.device)  # Initial position
    # Extract yaw from quaternion (w, x, y, z) = (0.9677, 0.0, 0.0, -0.2522)
    initial_quat = torch.tensor([0.9677, 0.0, 0.0, -0.2522], device=sim.device)
    cube_control_yaw = quat_to_yaw(initial_quat)  # Initial yaw angle in radians

    # Control speed
    pos_speed = 0.001  # meters per step
    yaw_speed = 0.01  # radians per step

    # Create buffers to store actions (will be set based on cube position)
    ...
    while simulation_app.is_running():
        # Handle keyboard input for cube control
        if keyboard_state["x_pos"]:
            cube_control_pos[0] += pos_speed
        if keyboard_state["x_neg"]:
            cube_control_pos[0] -= pos_speed
        if keyboard_state["y_pos"]:
            cube_control_pos[1] += pos_speed
        if keyboard_state["y_neg"]:
            cube_control_pos[1] -= pos_speed
        if keyboard_state["z_pos"]:
            cube_control_pos[2] += pos_speed
        if keyboard_state["z_neg"]:
            cube_control_pos[2] -= pos_speed
        if keyboard_state["yaw_pos"]:
            cube_control_yaw += yaw_speed
        if keyboard_state["yaw_neg"]:
            cube_control_yaw -= yaw_speed

        # Update cube position and orientation based on control state
        cube_quat = yaw_to_quat(cube_control_yaw)
        cube_pos_w = cube_control_pos.unsqueeze(0).repeat(scene.num_envs, 1) + scene.env_origins
        cube_quat_w = cube_quat.unsqueeze(0).repeat(scene.num_envs, 1)
        ...
        # Set IK target to cube position (relative to robot base)
        # Cube position in world frame relative to env_origin
        cube_pos_rel = cube_control_pos  # [3]
        cube_quat_rel = cube_quat  # [4]

        # Convert to robot base frame (subtract env_origin and offset)
        # The IK controller expects position relative to robot base
        ik_target_pos = cube_pos_rel - offset - cube_hand_offset  # Position relative to robot base
        ik_target_quat = cube_quat_rel  # Quaternion

        # Set IK command for all environments
        ik_commands[:, 0:3] = ik_target_pos.unsqueeze(0).repeat(scene.num_envs, 1)
        ...
        if count % 200 == 0:
            ...
            robot.reset()
            # reset actions - IK commands are already set above based on cube position
            joint_pos_des = joint_pos[:, robot_entity_cfg.joint_ids].clone()
            # reset controller
            diff_ik_controller.reset()
            diff_ik_controller.set_command(ik_commands)
        else:
            # Update IK controller command based on current cube position
            diff_ik_controller.set_command(ik_commands)
        ...
        if (count + 1) % 150 == 0:
            if not recording_complete:
                ...
                target_pos = cube_control_pos.unsqueeze(0) + scene.env_origins
                ...
                # Check if any environment has position error < 0.1
