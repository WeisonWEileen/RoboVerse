...
class SteppingWrapper(HumanoidBaseWrapper):
    ...
    def _init_target_wp(self, env_states: EnvState) -> None:
        self.ori_feet_pos = env_states.robots[self.scenario.robot.name][
            :, self.feet_indices, :2
        ].clone()  # [num_envs, 2, 2], two feet's original xy positions
        self.target_wp, self.num_pairs, self.num_wp = self.sample_fp(
            self.device, num_points=1000000, num_wp=10, ranges=self.cfg.command_ranges
    ...
    def _compute_observations(self, envstates: EnvState) -> None:
        ...
        feet_pos = envstates.robots[self.robot.name]["rigid_body_states"][
            :, self.feet_indices, :2
        ]  # [num_envs, 2, 2], two feet
