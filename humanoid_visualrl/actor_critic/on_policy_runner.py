...
class OnPolicyRunner:
    ...
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu", use_vision=False):
        ...
        # TODO: hard code here
        policy: ActorCriticCNN = policy_class(num_obs, num_privileged_obs, self.env.num_actions +1, action_masking=env.action_masking, **self.policy_cfg).to(
            self.device
        )
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False, run_name: str = None):  # noqa: C901
        # initialize writer
        if self.log_dir is not None and self.writer is None and not self.disable_logs:
            ...
            if self.logger_type == "neptune":
                ...
                self.writer = WandbSummaryWriter(log_dir=self.log_dir, flush_secs=10, cfg=self.cfg, run_name=run_name)
                self.writer.log_config(self.env.cfg, self.cfg, self.alg_cfg, self.policy_cfg)
